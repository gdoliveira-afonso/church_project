...
export function attendanceView(params) {
  ...
  <header class="sticky top-0 z-20 bg-white border-b border-slate-100">
    ...
    <div class="px-4 pb-3">
      ...
      <div class="flex gap-4 mt-3 text-sm font-medium">
        ${blockedMessage ? '' : `
        <span class="flex items-center gap-1.5 text-primary" id="cnt-p"><span class="w-2 h-2 rounded-full bg-primary"></span>0 Presente</span>
        <span class="flex items-center gap-1.5 text-slate-400" id="cnt-a"><span class="w-2 h-2 rounded-full bg-slate-300"></span>0 Ausente</span>
        `}
      </div>
    </div>
  ...
  <div class="flex-1 overflow-y-auto px-4 py-4 pb-28 space-y-3" id="att-list"></div>
  ...
  function render() {
    document.getElementById('att-list').innerHTML = mem.map(m => {
      ...
      return `<div class="bg-white rounded-xl p-4 border border-slate-100">
        ...
        <div class="grid grid-cols-3 gap-1.5 bg-slate-50 p-1 rounded-lg">
          <button class="att-btn flex flex-col items-center py-2 rounded text-xs font-medium transition ${s === 'present' ? 'bg-white shadow-sm border border-slate-200 text-primary' : 'text-slate-400 hover:bg-white/50'}" data-p="${m.id}" data-s="present"><span class="material-symbols-outlined text-base mb-0.5">check_circle</span>Presente</button>
          <button class="att-btn flex flex-col items-center py-2 rounded text-xs font-medium transition ${s === 'absent' ? 'bg-red-50 shadow-sm border border-red-100 text-red-600' : 'text-slate-400 hover:bg-white/50'}" data-p="${m.id}" data-s="absent"><span class="material-symbols-outlined text-base mb-0.5">cancel</span>Ausente</button>
          <button class="att-btn flex flex-col items-center py-2 rounded text-xs font-medium transition ${s === 'justified' ? 'bg-amber-50 shadow-sm border border-amber-100 text-amber-600' : 'text-slate-400 hover:bg-white/50'}" data-p="${m.id}" data-s="justified"><span class="material-symbols-outlined text-base mb-0.5">history_edu</span>Justific.</button>
        </div>
      </div>`;
    }).join('');
    if (!blockedMessage) {
      document.getElementById('cnt-p').querySelector('span:last-child').textContent = `${Object.values(att).filter(v => v === 'present').length} Presente`;
      document.getElementById('cnt-a').querySelector('span:last-child').textContent = `${Object.values(att).filter(v => v === 'absent').length} Ausente`;
